...
Trata autenticação, operadores da plataforma, usuários AD, grupos AD, OUs, computadores, GPOs, troca de senhas
...
# Importação defensiva do módulo de IA
try:
...
class SambaHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/api/platform-users":
            ...
            return self._send_json(200 if success else 400, {"success": True, "operators": platform_users})
        ...
        elif path == "/api/users/recent":
            limit = int(query.get("limit", [20])[0])
            success, recent_users, cmd_preview = samba_svc.get_recent_users(limit=limit)
            return self._send_json(200 if success else 400, {
                "success": success,
            ...
            success, groups, cmd_preview = samba_svc.list_groups()
            return self._send_json(200 if success else 400, {
                "success": success,
                "groups": groups,
                "cmd_preview": cmd_preview
    ...
    def do_POST(self):
        ...
        if path == "/api/system/power":
            ...
            success, msg = samba_svc.register_platform_user(username, password, full_name, role)
            return self._send_json(200 if success else 400, {
                "success": success,
                "message": msg if success else None,
                "error": msg if not success else None
        ...
        elif path == "/api/platform-users/password":
            username = data.get("username", "")
            ...
            success, msg = samba_svc.update_platform_user_password(username, new_password)
            return self._send_json(200 if success else 400, {
                "success": success,
                "message": msg if success else None,
                "error": msg if not success else None
            ...
            group_name = data.get("name", "") or data.get("group_name", "")
            description = data.get("description", "")

            if not group_name:
                return self._send_json(400, {"success": False, "error": "Nome do grupo é obrigatório."})

            success, msg, cmd_preview = samba_svc.create_group(group_name, description)
            return self._send_json(200 if success else 400, {
                "success": success,
                "message": msg if success else None,
                "error": msg if not success else None,
            ...
            if not group_name or not username:
                return self._send_json(400, {"success": False, "error": "Nome do grupo e do usuário são obrigatórios."})

            success, msg, cmd_preview = samba_svc.add_group_member(group_name, username)
            return self._send_json(200 if success else 400, {
                "success": success,
                "message": msg if success else None,
                "error": msg if not success else None,
            ...
            success, msg, cmd_preview = samba_svc.create_gpo(display_name, admin_user, admin_pass)
            return self._send_json(200 if success else 400, {
                "success": success,
                "message": msg if success else None,
                "error": msg if not success else None,
    ...
    def do_DELETE(self):
        ...
        if path == "/api/platform-users":
            ...
            group_name = data.get("name", "") or data.get("group_name", "")
            if not group_name:
                return self._send_json(400, {"success": False, "error": "Nome do grupo obrigatório."})
            success, msg, cmd_preview = samba_svc.delete_group(group_name)
            return self._send_json(200 if success else 400, {
                "success": success,
                "message": msg if success else None,
                "error": msg if not success else None,
            ...
            if not group_name or not username:
                return self._send_json(400, {"success": False, "error": "Nome do grupo e do usuário são obrigatórios."})
            success, msg, cmd_preview = samba_svc.remove_group_member(group_name, username)
            return self._send_json(200 if success else 400, {
                "success": success,
                "message": msg if success else None,
                "error": msg if not success else None,
            ...
            gpo_guid = data.get("guid", "") or data.get("gpo_guid", "")
            admin_user = data.get("admin_user", None)
            admin_pass = data.get("admin_pass", None)
            if not gpo_guid:
                return self._send_json(400, {"success": False, "error": "GUID ou nome da GPO é obrigatório."})
            success, msg, cmd_preview = samba_svc.delete_gpo(gpo_guid, admin_user, admin_pass)
            return self._send_json(200 if success else 400, {
                "success": success,
                "message": msg if success else None,
                "error": msg if not success else None,
