document.addEventListener('DOMContentLoaded', () => {
    ...
    const openNewGroupModalBtn = document.getElementById('open-new-group-modal');
    ...
    const inputUserGroup = document.getElementById('input-user-group');
    const inputGroupOu = document.getElementById('input-group-ou');
    ...
    const groupCmdPreview = document.getElementById('group-cmd-preview');
    ...
    document.getElementById('btn-quick-new-group')?.addEventListener('click', () => openModal(modalNewGroup));
    ...
    async function fetchGroups() {
        try {
            ...
            if (data.success) {
                ...
                populateUserGroupSelect(appState.groups);
                if (data.cmd_preview) addCommandToLog(data.cmd_preview);
            }
        } catch (err) {
            console.error('Erro ao buscar grupos do Active Directory:', err);
        }
    }
    ...
    function updateModeUI(isMock) {
        if (modeBadge) {
            if (isMock) {
                ...
                modeBadge.className = 'badge real Real';
    ...
    function renderGroups(groupsList) {
        ...
        if (groupsList.length === 0) {
            groupsTableBody.innerHTML = `<tr><td colspan="5" class="text-center text-muted">Nenhum grupo encontrado.</td></tr>`;
            return;
        }
        ...
        groupsList.forEach(g => {
            const tr = document.createElement('tr');
            tr.innerHTML = `
                <td><strong style="color: #60a5fa; font-family: var(--font-mono);">👥 ${g.name}</strong></td>
                <td><span style="color: var(--text-secondary);">${g.description || '-'}</span></td>
                <td><span class="badge ${g.system ? 'system' : 'user-type'}">${g.scope || 'Global / Segurança'}</span></td>
                <td><span class="badge computer">${g.ou || 'CN=Users'}</span></td>
                <td>
                    <div class="table-actions">
                        ${g.system ? '<span class="text-muted" style="font-size: 0.8rem;">Protegido</span>' : 
                        `<button class="btn btn-sm btn-danger btn-delete-group" data-name="${g.name}">Excluir</button>`}
                    </div>
                </td>
            `;
    ...
    function populateOuSelects(ousList) {
        ...
        if (inputGroupOu) inputGroupOu.innerHTML = `<option value="">(Raiz do Domínio / Default)</option>`;
        ...
        ousList.forEach(o => {
            ...
            if (inputGroupOu) inputGroupOu.innerHTML += `<option value="${o.dn}">${o.name} (${o.dn})</option>`;
    ...
    function populateUserGroupSelect(groupsList) {
        const selectGroup = document.getElementById('input-user-group');
        if (!selectGroup) return;

        selectGroup.innerHTML = '<option value="">(Padrão: Domain Users)</option>';

        if (Array.isArray(groupsList)) {
            groupsList.forEach(group => {
                const groupName = typeof group === 'string' ? group : group.name;

                if (groupName && groupName.toLowerCase() !== 'domain users') {
    ...
    async function deleteGroup(name) {
        if (!confirm(`Tem certeza que deseja excluir o grupo '${name}' do Active Directory?\nComando CLI: samba-tool group delete "${name}"`)) return;
        ...
        try {
            ...
            if (data.success) {
                ...
                showToast(data.error || 'Erro ao excluir grupo.', 'error');
            }
        } catch (err) {}
    }
    ...
    formNewUser?.addEventListener('submit', async (e) => {
        ...
        const group = document.getElementById('input-user-group')?.value || '';

        try {
            const res = await authenticatedFetch('/api/users', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ username, password, given_name, surname, mail, ou, group })
            });
            const data = await res.json();
            if (data.success) {
                showToast(data.message, 'success');
    ...
    formNewGroup?.addEventListener('submit', async (e) => {
        ...
        const description = document.getElementById('input-group-desc')?.value;
        const ou = inputGroupOu ? inputGroupOu.value : '';

        try {
            const res = await authenticatedFetch('/api/groups', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ name, description, ou })
            });
            const data = await res.json();
            if (data.success) {
                showToast(data.message, 'success');
    ...
    document.getElementById('input-user-group')?.addEventListener('change', updateCmdPreviews);
    document.getElementById('input-group-name')?.addEventListener('input', updateCmdPreviews);
    document.getElementById('input-group-ou')?.addEventListener('change', updateCmdPreviews);
    ...
    function updateCmdPreviews() {
        if (userCmdPreview) {
            ...
            const uGroup = document.getElementById('input-user-group')?.value;

            let cmd = `sudo samba-tool user create "${uName}" '*****'${uOu}`;
            if (uGroup) {
                cmd += ` && sudo samba-tool group addmembers "${uGroup}" "${uName}"`;
            }
            userCmdPreview.textContent = cmd;
        }

        if (groupCmdPreview) {
            const gName = document.getElementById('input-group-name')?.value || 'nome_do_grupo';
            const gOu = (inputGroupOu && inputGroupOu.value) ? ` --groupou="${inputGroupOu.value}"` : '';
            groupCmdPreview.textContent = `sudo samba-tool group add "${gName}"${gOu}`;
        }
    ...
    openNewGroupModalBtn?.addEventListener('click', () => openModal(modalNewGroup));
    ...
    searchGroupsInput?.addEventListener('input', (e) => {
        ...
        const filtered = appState.groups.filter(g => 
            ...
            (g.ou && g.ou.toLowerCase().includes(query))
        );
    ...
    function renderSystemInfo(info) {
        ...
        // 5. VELOCIDADE DA INTERNET & REDE EM TEMPO REAL
        if (info.network) {
            ...
            // Formata numérico para Mbps se vier do backend
            const downVal = typeof info.network.download_speed === 'number' 
                ? `${info.network.download_speed.toFixed(1)} Mbps` 
                : (info.network.download_speed || '0.0 Mbps');

            const upVal = typeof info.network.upload_speed === 'number' 
                ? `${info.network.upload_speed.toFixed(1)} Mbps` 
                : (info.network.upload_speed || '0.0 Mbps');

            if (netDown) netDown.textContent = downVal;
            if (netUp) netUp.textContent = upVal;
    ...
    // Atualização em tempo real do tráfego de rede e métricas do sistema a cada 2 segundos (2000ms)
    setInterval(() => {
        if (appState.token) {
            fetchSystemInfo();
        }
