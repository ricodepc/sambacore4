...
# Dependências de monitoramento e banco
try:
    import psutil
    HAS_PSUTIL = True
except ImportError:
...
class SambaService:
    ...
    def create_user(self, username, password, ou=None, surname=None, given_name=None, mail=None, group=None):
        ...
        if self.mock_mode:
            ...
            if group and group.strip():
                grp_success, grp_msg, grp_cmd = self.add_group_member(group.strip(), username)
                if grp_success:
                    msg += f" e adicionado ao grupo '{group.strip()}'!"
                else:
                    msg += f" (Aviso: {grp_msg})"
                cmd_preview += f" && {grp_cmd}"

            return True, msg, cmd_preview

        success, msg, cmd_str = self._run_cmd(args)

        if success and group and group.strip():
            grp_success, grp_msg, grp_cmd = self.add_group_member(group.strip(), username)
            if grp_success:
                msg += f" e adicionado ao grupo '{group.strip()}'!"
            else:
                msg += f" (Aviso: Usuário criado, mas falhou ao adicionar ao grupo: {grp_msg})"
            cmd_str += f" && {grp_cmd}"
    ...
    # --- MONITORAMENTO DO SISTEMA E PING ---

    def _get_ping_ms(self, host="8.8.8.8"):
        """Executa um ping no sistema para obter a latência em ms."""
        try:
            res = subprocess.run(
                ["ping", "-c", "1", "-W", "1", host],
                capture_output=True,
            ...
            if res.returncode == 0:
                for line in res.stdout.splitlines():
                    if "rtt" in line or "round-trip" in line:
                        ...
                        return round(float(avg_ping), 1)
            return 12.0  # Valor padrão de fallback
        except Exception:
            return 0.0

    def get_system_info(self):
        """Retorna informações detalhadas de hardware e métricas em tempo real do sistema."""
        cpu_model = ""
        ...
        cpu_percent = psutil.cpu_percent(interval=0.1) if HAS_PSUTIL else 15.0
        ...
        # Medição real de Rede (Download, Upload, Ping e Status)
        rx_mbps = 0.0
        tx_mbps = 0.0
        link_status = "Online (1 Gbps Full Duplex)"
        ...
        if HAS_PSUTIL:
            try:
                ...
                rx_mbps = round(((io_end.bytes_recv - io_start.bytes_recv) * 5 * 8) / 1_000_000, 1)
                tx_mbps = round(((io_end.bytes_sent - io_start.bytes_sent) * 5 * 8) / 1_000_000, 1)

                if_stats = psutil.net_if_stats()
                for iface, stat in if_stats.items():
                    if iface != "lo" and stat.isup:
                        speed = f"{stat.speed // 1000} Gbps" if stat.speed >= 1000 else f"{stat.speed} Mbps"
                        duplex = "Full Duplex" if stat.duplex == psutil.NIC_DUPLEX_FULL else "Half Duplex"
                        link_status = f"Online ({speed} {duplex})"
                        break
            except Exception:
                pass

        ping_ms = self._get_ping_ms("8.8.8.8")
        ...
        return {
            ...
            "network": {
                "download_speed": f"{rx_mbps} Mbps",
                "upload_speed": f"{tx_mbps} Mbps",
                "latency": f"{ping_ms} ms",
                "interface_status": link_status,
                "status": "Conectado",
